...
impl Default for ConversionOptions {
  fn default() -> Self {
    Self {
      package_name: wit_encoder::PackageName::new("webidl", "web-bindings", None),
  ...
  for item in webidl {
    match item {
      ...
      Definition::PartialInterface(wi_interface) => {
        ...
        state.interface_members_to_functions(&resource_name, &wi_interface.members.body, false)?;
      }
      ...
      Definition::Interface(wi_interface) => {
        ...
        if is_singleton {
          ...
          if let Some(inheritance) = wi_interface.inheritance {
            ...
            match state.resource_inheritance {
              ResourceInheritance::AsBaseMethod => {
                resource.func({
                  let mut func =
                    wit_encoder::ResourceFunc::method(format!("as-{}", base_name.raw_name()));
          ...
          if let Some(inheritors) = state.inheritors_waiting_for_base.remove(&interface_name) {
      ...
      Definition::Dictionary(dict) => {
        ...
        if let Some(inheritance) = dict.inheritance {
          ...
          let base = state.interface.items().iter().find_map(|t| match t {
            InterfaceItem::TypeDef(type_def) => match type_def.kind() {
              wit_encoder::TypeDefKind::Record(record) if type_def.name() == &base_name => {
                Some(record)
              }
              _ => None,
            },
                ...
                InterfaceItem::TypeDef(type_def) => {
                  if type_def.name() == &inheritor {
                    match type_def.kind_mut() {
                      wit_encoder::TypeDefKind::Record(record) => Some(record),
    ...
    for member in members {
      match member {
        weedle::interface::InterfaceMember::Const(const_) => {
          fn const_type_to_wi_type(const_type: weedle::types::ConstType) -> weedle::types::Type {
            use weedle::types::{ConstType, SingleType, Type};
            match const_type {
              ConstType::Integer(t) => Type::Single(SingleType::NonAny(t.into())),
              ConstType::FloatingPoint(t) => Type::Single(SingleType::NonAny(t.into())),
          ...
          match &mut functions {
            ...
            Funcs::Resource(functions) => {
              let mut function = wit_encoder::ResourceFunc::static_(const_name.clone());
              function.set_results(type_);
              functions.insert(const_name.into(), vec![function]);
            }
          }
        ...
        weedle::interface::InterfaceMember::AsyncIterable(_) => {
          handle_unsupported(interface_name, "async iterable", &self.unsupported_features);
          continue;
        }
        ...
        weedle::interface::InterfaceMember::Attribute(attr) => {
          ...
          if let Type::Single(SingleType::NonAny(NonAnyType::Promise(promise))) = &attr.type_.type_
          {
        ...
        weedle::interface::InterfaceMember::Constructor(constructor) => match &mut functions {
          Funcs::Standalone(_) => {
            bail!(
          ...
          if let Some(identifier) = operation.identifier {
            ...
            let results = match &operation.return_type {
              ...
              weedle::types::ReturnType::Type(type_) => {
                ...
                // TODO: remove the `is_undefined_promise` once we have proper Future support
                let mut is_undefined_promise = false;
                if let Type::Single(SingleType::NonAny(NonAnyType::Promise(promise))) = type_ {
                  if let ReturnType::Undefined(_) = &*promise.generics.body {
                    is_undefined_promise = true;
                  }
                }
      ...
      Funcs::Resource(mut functions) => {
        ...
        let resource = self
          ...
          .filter_map(|td| match td {
            ...
            wit_encoder::InterfaceItem::Use(_) | wit_encoder::InterfaceItem::Function(_) => None,
          })
        ...
        match resource {
          ...
          Some(resource) => {
            ...
            if let Some(functions) = self.waiting_resource_methods.remove(&interface_name) {
...
fn namespace_to_interface_member(
  ...
  match mixin_member {
    ...
    weedle::namespace::NamespaceMember::Operation(operation) => {
      weedle::interface::InterfaceMember::Operation(weedle::interface::OperationInterfaceMember {
    ...
    weedle::namespace::NamespaceMember::Attribute(attribute) => {
      weedle::interface::InterfaceMember::Attribute(weedle::interface::AttributeInterfaceMember {
...
fn mixin_to_interface_member(
  ...
  match mixin_member {
    weedle::mixin::MixinMember::Const(const_) => weedle::interface::InterfaceMember::Const(const_),
    weedle::mixin::MixinMember::Operation(operation) => {
      weedle::interface::InterfaceMember::Operation(weedle::interface::OperationInterfaceMember {
        attributes: operation.attributes,
        modifier: operation
          .stringifier
          .map(|stringifier| weedle::interface::StringifierOrStatic::Stringifier(stringifier)),
        special: None,
        return_type: operation.return_type,
        identifier: operation.identifier,
        args: operation.args,
